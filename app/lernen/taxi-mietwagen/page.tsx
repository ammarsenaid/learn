...
export default async function TaxiLernenPage() {
  ...
  return (
    <main className="min-h-screen bg-[#03111f] px-5 py-10 text-slate-100"><div className="mx-auto max-w-6xl space-y-6">
      ...
      <p className="mt-3 text-sm text-slate-300">Kapitel {chapters.length} · Lektionen {lessons.length} · Lernkarten {flashcards.length} · Fragen {questions.length} · Glossar {glossary.length}</p></section>
      {chapters.map((chapter) => <section key={chapter.id} className="rounded-2xl border border-[#1e385c] bg-[#0a2039] p-5"><h2 className="text-xl font-semibold text-white">{chapter.title}</h2><p className="text-sm text-slate-300">{chapter.description}</p>
