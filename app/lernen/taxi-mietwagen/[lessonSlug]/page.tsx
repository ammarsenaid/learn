...
export default async function LessonDetailPage({ params }: { params: Promise<{ lessonSlug: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#03111f] px-5 py-10 text-slate-100"><div className="mx-auto max-w-4xl space-y-6">
      ...
      {flashcards.length > 0 ? <section className="rounded-xl border border-[#214267] bg-[#0b2645] p-5"><h2 className="text-lg font-semibold">Zugehörige Lernkarten</h2><ul className="mt-3 space-y-2">{flashcards.map((f) => <li key={f.id} className="rounded border border-[#2a4c77] p-3"><Link href={`/lernen/taxi-mietwagen/karten/${f.id}`} className="text-sky-200 hover:underline">{f.front_de}</Link></li>)}</ul></section> : null}
      {questions.length > 0 ? <section className="rounded-xl border border-[#214267] bg-[#0b2645] p-5"><h2 className="text-lg font-semibold">Zugehörige Fragen</h2><ul className="mt-3 space-y-2">{questions.slice(0,8).map((q) => <li key={q.id} className="rounded border border-[#2a4c77] p-3"><Link href={`/lernen/taxi-mietwagen/fragen/${q.id}`} className="text-sky-200 hover:underline">{q.question_de}</Link></li>)}</ul><Link href="/lernen/taxi-mietwagen/simulation" className="mt-4 inline-flex rounded-lg bg-[#f3c76a] px-4 py-2 font-semibold text-[#03111f]">Simulation starten</Link></section> : null}
