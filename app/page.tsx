...
function Panel({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <section className={`rounded-[24px] border border-white/10 bg-[rgba(8,24,58,0.88)] shadow-[0_24px_70px_rgba(1,12,32,.45)] ${className}`}>{children}</section>;
}

const icon = "inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-[#4EA1FF]";

export default function Page() {
  return (
    <div className="min-h-screen bg-[#07162F] text-[#F8FAFC]">
      <div className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(circle_at_60%_16%,rgba(78,161,255,.3),transparent_30%),radial-gradient(circle_at_20%_34%,rgba(6,52,124,.2),transparent_48%)]" />
      <div className="mx-auto max-w-[1360px] px-6 pb-16 pt-4 lg:px-8">
        <nav className="mb-8 flex min-h-[82px] items-center justify-between rounded-[22px] border border-white/15 bg-[rgba(7,22,48,.88)] px-5 shadow-[0_10px_40px_rgba(0,0,0,.4)] md:px-7">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-[#F4B73F]/45 bg-[#0C1F4A] text-[#F4B73F]">✦</div>
            <div>
              <p className="text-3xl font-bold leading-none">FachkundePilot</p>
              <p className="text-xs text-[#8EA0C0]">Prüfungsorientiertes Lernen</p>
            </div>
          </div>
          <div className="hidden gap-10 text-[#C7D2E5] lg:flex">{navItems.map((x) => <span key={x}>{x}</span>)}</div>
          <div className="flex items-center gap-3"><button className="hidden rounded-xl border border-white/15 px-4 py-2 md:block">DE</button><button className="rounded-xl bg-[#F4B73F] px-7 py-3 font-semibold text-[#07162F] shadow-[0_8px_20px_rgba(244,183,63,.3)]">Jetzt starten</button></div>
        </nav>

        <section className="grid items-center gap-8 pb-8 lg:grid-cols-[1.02fr_1.18fr]">
          <div>
            <p className="inline-flex rounded-full border border-[#4EA1FF]/55 bg-[rgba(10,27,63,.82)] px-4 py-2 text-sm">Die Lernplattform für offizielle Sach- und Fachkundeprüfungen</p>
            <h1 className="mt-5 text-5xl font-bold leading-[1.03] xl:text-[68px]">Bestehe deine Prüfung.<br />Mit <span className="text-[#F4B73F]">Klarheit,<br />Struktur und System.</span></h1>
          ...
          <div className="grid gap-4 xl:grid-cols-[1fr_220px]">
            <Panel className="border-[#4EA1FF]/35 p-4 shadow-[0_0_0_1px_rgba(78,161,255,.2),0_0_35px_rgba(78,161,255,.2)]">
              <div className="flex items-center justify-between rounded-xl border border-white/10 bg-[#091A3C] px-3 py-2 text-sm"><span>FachkundePilot Dashboard</span><span>⌕  🔔  ●</span></div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">{heroStats.map((s) => <Panel key={s.title} className="p-4"><p className="text-2xl font-bold">{s.title}</p><p className="text-sm text-[#C7D2E5]">{s.value}</p></Panel>)}</div>
          </div>
        </section>
        ...
        <Panel className="mb-7 p-5 md:p-7"><div className="mb-6 flex items-end justify-between"><div><h2 className="text-5xl font-bold">Zertifikate entdecken</h2><p className="text-[#C7D2E5]">Wähle dein Ziel und starte mit einer klaren Prüfungsvorbereitung.</p></div><span className="text-[#4EA1FF]">Alle Zertifikate ansehen →</span></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{certificates.map(([t,s,d,st,b]) => <article key={t} className="rounded-[20px] border border-white/10 bg-[rgba(10,27,63,.82)] p-5 transition hover:border-[#4EA1FF]/35"><div className="mb-3 flex justify-between"><div className={icon}>⬢</div><span className="rounded-full border border-white/20 px-2 py-1 text-xs">{b}</span></div><h3 className="text-[34px] font-semibold leading-tight">{t}</h3><p className="mt-1 text-sm text-[#C7D2E5]">{s}</p><p className="mt-2 text-sm text-[#8EA0C0]">{d}</p><p className="mt-3 text-sm">{st}</p><button className="mt-2 text-[#4EA1FF]">Mehr erfahren →</button></article>)}</div></Panel>
        ...
        <div className="mb-7 grid gap-6 xl:grid-cols-2">
          ...
          <Panel className="p-6"><h3 className="text-5xl font-bold">Das sagen unsere Teilnehmenden</h3><div className="mt-5 grid gap-3 md:grid-cols-3 xl:grid-cols-1">{testimonials.map(([n,q])=><div key={n} className="rounded-xl border border-white/10 p-3"><p className="font-semibold">{n}</p><p className="text-[#F4B73F]">★★★★★</p><p className="text-sm text-[#C7D2E5]">“{q}”</p></div>)}</div></Panel>
        </div>

        <Panel className="border-[#F4B73F]/35 p-6"><div className="grid gap-4 xl:grid-cols-[1.2fr_.7fr_360px]"><div><p className="inline-flex rounded-full border border-[#F4B73F]/50 px-3 py-1 text-sm">Dein Erfolg beginnt jetzt</p><h3 className="mt-3 text-5xl font-bold">Starte heute mit deiner Prüfungsvorbereitung.</h3><p className="mt-3 text-[#C7D2E5]">Lerne mit klaren Lernpfaden, smarten Lernkarten und realistischen Prüfungen — für deinen sicheren Erfolg.</p><ul className="mt-4 space-y-2 text-[#C7D2E5]"><li>Zugriff auf alle Kurse & Zertifikate</li><li>Prüfungssimulationen & Lernkarten</li><li>Mehrsprachige Inhalte</li><li>Lernen auf allen Geräten</li><li>DSGVO-konform & sicher</li></ul></div><div className="space-y-3"><div className="rounded-xl border border-white/15 p-3">14 Tage Geld-zurück-Garantie</div><div className="rounded-xl border border-white/15 p-3">Jederzeit kündbar</div><div className="rounded-xl border border-white/15 p-3">Sofortiger Zugriff nach Registrierung</div></div><div className="rounded-[20px] border border-[#F4B73F]/45 bg-[#F8F3E8] p-5 text-[#08142B]"><p className="inline-flex rounded-full bg-[#F4B73F] px-3 py-1 font-semibold">Premium</p><p className="mt-3 font-semibold">Alles inklusive für deinen Prüfungserfolg</p><p className="mt-3 text-5xl font-bold">Ab 9,90 € <span className="text-lg">/ Monat</span></p><p>Monatlich kündbar</p><button className="mt-4 w-full rounded-xl bg-[#F4B73F] px-4 py-3 font-semibold">Jetzt starten →</button><p className="mt-2 text-center text-sm">7 Tage kostenlos testen</p></div></div></Panel>
      </div>
