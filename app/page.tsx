...
export const metadata: Metadata = {
  title: "FachkundePilot — Deutsche Fachkundeprüfungen gezielt vorbereiten",
  description:
...
const painPoints = [
  {
    icon: "warning",
    title: "Unterlagen sind verstreut",
    text: "PDFs, Notizen und alte Lernfragen liegen oft nebeneinander, aber ohne klare Reihenfolge.",
  },
  {
    icon: "globe",
    title: "Deutsch bleibt die Hürde",
    text: "Die Prüfung ist deutsch. Wer die Fachbegriffe nicht sauber versteht, verliert unnötig Punkte.",
  },
  {
    icon: "target",
    title: "Zu wenig Prüfungsdruck",
    text: "Nur lesen reicht nicht. Man muss Fragen unter Zeitdruck verstehen, entscheiden und auswerten.",
  },
  {
    icon: "brain",
    title: "Rechnen blockiert viele",
    text: "Kostenrechnung, Prozent, Dreisatz und Formeln brauchen klare Logik statt Auswendiglernen.",
  },
] as const;
...
const methodSteps = [
  {
    number: "01",
    title: "Deutsch sehen",
    text: "Originale Begriffe, typische Prüfungsformulierungen und wichtige Regeln zuerst auf Deutsch.",
  },
  {
    number: "02",
    title: "Einfach verstehen",
    text: "Danach Erklärung in einfacher Sprache und optional mit Arabisch, Englisch oder Türkisch.",
  },
  {
    number: "03",
    title: "Aktiv sichern",
    text: "Definitionen, Regeln, Formeln und Falllogik werden als Karten und Mini-Fragen trainiert.",
  },
  {
    number: "04",
...
const pricingPlans = [
  ...
  },
  {
    ...
    period: "",
    label: "B2B",
    description: "Für Schulen, Bildungsträger und Teams.",
    benefits: [
      "Nutzerverwaltung",
      "Dozentenrollen",
      "Content-Verwaltung",
...
const learnerScenarios = [
  [
    "AR",
    "Deutsch verstehen",
    "Für Lernende mit anderer Muttersprache",
    "Deutsche Prüfungsbegriffe bleiben sichtbar, während einfache Erklärungen beim Verstehen helfen.",
  ],
  [
    "EX",
    "Prüfungsmodus",
    "Für Lernende kurz vor dem Termin",
    "Simulationen zeigen, ob man Fragen nur kennt oder wirklich unter Zeitdruck lösen kann.",
  ],
  [
    "RE",
    "Rechnen trainieren",
    "Für Kostenrechnung und Prozentaufgaben",
    "Rechenwege werden Schritt für Schritt aufgebaut, damit die Logik hinter der Aufgabe klar wird.",
  ],
] as const;
      ...
      >
        {navItems.map((item) => (
          <a
            key={item.label}
            href={item.href}
            className="shrink-0 rounded-xl bg-white/[0.06] px-4 py-2 text-xs font-black text-slate-300 transition hover:bg-white/10 hover:text-white"
          >
            {item.label}
          </a>
        ))}
        ...
        <div>
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-[1.18fr_.82fr]">
            ...
            <div className="rounded-xl border border-yellow-400/25 bg-yellow-400/10 p-3">
              ...
              <a
                href="#methode"
                className="mt-3 block rounded-lg bg-gradient-to-b from-sky-400 to-blue-600 px-3 py-2 text-center text-xs font-black text-white transition hover:-translate-y-0.5"
              >
                Lernmethode ansehen
              </a>
            </div>
...
function SideStats() {
  const items = [
    ...
    ["analytics", "Smart Review", "Fehler gezielt wiederholen"],
      ...
      <div className="grid gap-8 rounded-3xl border border-rose-400/20 bg-gradient-to-br from-[#111827]/90 to-[#071a30]/90 p-7 shadow-[0_18px_55px_rgba(0,0,0,0.35)] lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {painPoints.map((point) => (
            <article
              key={point.title}
              className="rounded-2xl border border-slate-600/40 bg-[#081d36] p-5 transition hover:border-rose-300/50"
            >
  ...
  return (
    <section id="zertifikate" className="mx-auto max-w-[1180px] px-4 pt-14 lg:pt-20">
      <div className="mb-6 flex items-end justify-between gap-4">
        ...
        <a href="#preise" className="hidden text-sm font-black text-sky-400 transition hover:text-sky-300 md:block">
        ...
        {certificates.map((item, index) => (
          ...
          >
            ...
            <div className="relative mt-5 space-y-2">
              ...
              <p className="text-xs font-black text-slate-500">{item.languages}</p>
            </div>
    ...
    <section className="mx-auto max-w-[1180px] px-4 pt-8 lg:pt-10">
      <div className="grid gap-4 rounded-2xl border border-slate-600/40 bg-[#071a30]/88 p-5 md:grid-cols-3">
        {featureStrip.map(([icon, title, text]) => (
          <div key={title} className="flex items-start gap-3 rounded-xl p-2 transition hover:bg-white/[0.03]">
            <IconBox icon={icon as IconName} />
            <div>
    ...
    <section id="methode" className="mx-auto max-w-[1180px] px-4 pt-14 lg:pt-20">
      <div className="grid gap-8 rounded-3xl border border-slate-600/40 bg-gradient-to-br from-[#071a30] to-[#0a2038] p-7 lg:grid-cols-[0.8fr_1.2fr]">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black leading-tight tracking-[-0.05em] text-white">
            Aus jedem Fehler wird der nächste Lernschritt.
          </h2>
          <p className="mt-4 text-base leading-7 text-slate-300">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {methodSteps.map((step) => (
            <article
              key={step.title}
              className="rounded-2xl border border-slate-600/40 bg-[#081d36] p-5 transition hover:border-yellow-400/40"
            >
...
function StepsAndScenarios() {
  const steps = [
    ["1", "Zertifikat wählen", "Wähle dein Prüfungsziel und starte mit einem klaren Lernpfad."],
    ["2", "Gezielt lernen", "Lerne Kapitel, Karten, Rechenlogik und prüfungsnahe Fragen."],
    ["3", "Sicherer werden", "Simuliere die Prüfung und schließe gezielt deine Schwächen."],
  ] as const;
      ...
      <div className="rounded-3xl border border-slate-600/40 bg-[#071a30]/88 p-7">
        <div className="flex items-end justify-between gap-4">
          ...
          <p className="hidden text-sm font-black text-yellow-300 sm:block">
            Praxisnah · Klar · Prüfungsorientiert
          </p>
        </div>

        <div className="mt-5 grid gap-4 md:grid-cols-3">
          {learnerScenarios.map(([initials, name, label, quote]) => (
            <article
              key={name}
          ...
          <article
            ...
            className={`rounded-2xl border p-6 transition duration-300 hover:-translate-y-1 ${
              plan.featured
                ...
                : "border-slate-600/40 bg-[#071a30]/88 hover:border-sky-400/40"
            }`}
          >
            <p
              className={`w-fit rounded-full px-3 py-1 text-[10px] font-black ${
      ...
      <div className="grid gap-7 rounded-3xl border border-slate-600/40 bg-[#071a30]/88 p-7 lg:grid-cols-[0.7fr_1.3fr]">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {faqItems.map((item) => (
            <article
              key={item.question}
              className="rounded-2xl border border-slate-600/40 bg-[#081d36] p-5 transition hover:border-yellow-400/35"
            >
      ...
      <div className="mx-auto grid max-w-[1180px] gap-10 px-4 py-12 text-sm text-slate-400 md:grid-cols-[1.5fr_repeat(4,1fr)]">
        <div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {[["Kontakt", "#kontakt"], ["FAQ", "#faq"], ["Preise", "#preise"], ["Demo", "#demo"]].map(([item, href]) => (
              <a
                key={item}
                href={href}
                aria-label={`Zu ${item}`}
