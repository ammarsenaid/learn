...
export type CmsPricingPlan = { id: string; name: string; price: string; period?: string | null; description?: string | null; features?: string[] | string | null; highlighted?: boolean | null };

function mapCmsCertificate(item: CmsCertificate): Certificate {
  return {
    slug: item.slug,
    title: item.title,
...
export async function getCertificates(): Promise<Certificate[]> {
  try {
    ...
    return items.map(mapCmsCertificate);
  } catch (error) {
